...
OSM <- st_read("DATA/greater-london-251102-free.shp/gis_osm_pois_a_free_1.shp")%>%
  filter(fclass == 'hotel') %>% #filter only for hotel
  st_transform(.,27700)#transform to 27700 gis
...
```{r map4, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
tm1 <- tm_shape(Accomodation_contained) +
    tm_polygons(
...
```{r map5, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
...
```{r wrangling20, include=FALSE}
tmap_save(tm2, "DATA/1_facet.png", width = 8, height = 6, units="in", dpi = 300)
```

```{r map7, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
qtm(UK_outline)
```

```{r wrangling10, include=FALSE}
...
```{r map9, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
